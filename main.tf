...
resource "hyperv_network_switch" "Access_to_LAN" {
  name                                    = "Access_to_LAN"
  notes                                   = "Access_to_LAN"
...
resource "hyperv_machine_instance" "ubuntu_vm" {
  name              = "Kubernetes_UbuntuLTS"  // Give your VM a cool name
  //source_image_path = "/path/to/ubuntu.vhdx" // Point to your Ubuntu VHDX file
  /*ram               = "6GB"  // Adjust RAM as needed, e.g., "4GB", "8GB", etc.*/
  memory_maximum_bytes = 6000000000
  memory_minimum_bytes = 3000000000
  memory_startup_bytes = 3000000000
  processor_count      = 6  // Number of CPUs you want to allocate
  /*switch_name          = "Access_to_LAN"  // Your Hyper-V switch name*/
  dynamic_memory  = "false" //aka dynamic memory
  /*enable_secure_boot = false //why the hassle*/
  ...
  /*vm_firmware {
    ...
    boot_order {
      ...
      network_adapter_name = "Access_to_LAN"
    }*/

  }

   /*disk {
    size     = "50GB"  // Adjust disk size as needed
    bus_type = "SCSI"  // Choose "SATA" or "SCSI"
  }*/

  // Optional: Add Cloud-Init or other provisioning here
  // e.g., cloud_init = file("path/to/cloud-init.yaml")
   ...
   /*cloud_init {
    ...
    command = <<EOT
    # PowerShell commands to attach ISO and set boot order
    Set-VMDvdDrive -VMName Kubernetes_UbuntuLTS -Path "C:\\Users\\Vivek\\Downloads\\ubuntu-22.04.3-desktop-amd64.iso"
    Set-VMFirmware -VMName Kubernetes_UbuntuLTS -FirstBootDevice $(Get-VMDvdDrive -VMName Kubernetes_UbuntuLTS)
  EOT
    }*/
